...
export default function Page() {
  ...
  const touchStartXVideo = useRef<number | null>(null);
  const touchStartXStudy = useRef<number | null>(null);
  ...
  const openModal = useCallback((videoId: string, trigger: HTMLElement) => {
  ...
  const handleStudyTouchStart = (event: ReactTouchEvent<HTMLDivElement>) => {
    touchStartXStudy.current = event.changedTouches[0]?.clientX ?? null;
  };

  const handleStudyTouchEnd = (event: ReactTouchEvent<HTMLDivElement>) => {
  ...
  const handleStudyKeyDown = (event: ReactKeyboardEvent<HTMLDivElement>) => {
    if (event.key === "ArrowRight") {
      event.preventDefault();
      nextStudy();
    } else if (event.key === "ArrowLeft") {
      ...
      <section className={styles.section} aria-labelledby="featured-videos-heading">
        <div className="container">
          <div className={styles.sectionHeader}>
            ...
            <p>
              Watch customers describe how DSN partnered with their teams to unlock revenue momentum.
            </p>
          </div>
